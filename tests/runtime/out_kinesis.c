...
/* It writes a big JSON message (copied from TD test) */
void flb_test_firehose_success_custom_endpoint(void)
{
    int ret;
    flb_ctx_t *ctx;
    int in_ffd;
    ...
    flb_output_set(ctx, out_ffd,"match", "*", NULL);
    flb_output_set(ctx, out_ffd,"region", "us-west-2", NULL);
    flb_output_set(ctx, out_ffd,"endpoint", "kinesis.us-west-2.amazonaws.com/", NULL);
    flb_output_set(ctx, out_ffd,"stream", "fluent", NULL);
    flb_output_set(ctx, out_ffd,"time_key", "time", NULL);
    flb_output_set(ctx, out_ffd,"Retry_Limit", "1", NULL);
...
TEST_LIST = {
    ...
    {"success_custom_endpoint", flb_test_firehose_success_custom_endpoint },
